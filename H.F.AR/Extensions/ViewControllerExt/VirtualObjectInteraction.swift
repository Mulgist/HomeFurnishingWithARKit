...
// Abstract: Coordinates movement and gesture interactions with virtual objects.
...
class VirtualObjectInteraction: NSObject, UIGestureRecognizerDelegate {
    ...
    // The object that has been most recently intereacted with. The 'selectedObject' can be moved at any time with the tap gesture.
    ...
    @objc func didRotate(_ gesture: UIRotationGestureRecognizer) {
        ...
        // - Note: For looking down on the object (99% of all use cases), we need to subtract the angle. To make rotation also work correctly when looking from below the object one would have to flip the sign of the angle depending on whether the object is above or below the camera...
	...
	private func translate(_ object: VirtualObject, basedOn screenPos: CGPoint, infinitePlane: Bool, allowAnimation: Bool) {
        guard let cameraTransform = sceneView.session.currentFrame?.camera.transform,
			let result = sceneView.smartHitTest(screenPos, infinitePlane: infinitePlane, objectPosition: object.simdWorldPosition, allowedAlignments: object.allowedAlignments) else { return }
        ...
        // Plane hit test results are generally smooth. If we did *not* hit a plane, smooth the movement to prevent large jumps.
