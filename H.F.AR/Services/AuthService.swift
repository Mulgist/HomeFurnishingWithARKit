...
class AuthService {
    ...
    func setAndLoadUserInfoById(completion: @escaping CompletionHandler) {
        // Web Request
        Alamofire.request("\(BASE_URL)\(REQUEST_SUFFIX)method=getloginuserinfo&user_id=\(userId)", method: .get, parameters: nil, encoding: JSONEncoding.default, headers: nil).responseJSON { (response) in
